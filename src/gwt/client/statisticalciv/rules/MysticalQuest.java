...
public class MysticalQuest implements PBaseRule{
	protected static final class FightVillage extends OObject {
		...
		public Returnable execute(FullMapData fullMapData, LivingBeing person) {
			...
			Demographics demo = DemographicRule.getDemo(person.getParent());
			if(demo == null){
				return null;
			}
			double difference=Demographics.compareCultureHighest(getHome(person),person.getParent());
			if(difference < .5){
				return null;
			}
			...
			if(demo.getDouble(VConstants.size) < 20){
				...
				if(nearestFarm != null){
					if(!isHated(nearestFarm, person)){
						nearestFarm.getMapData(VConstants.gate).getListCreate(VConstants.hatred).add(getHome(person).getPosition());	
					}
				}
	...
	public static boolean isHated(HashMapData nearestFarm, LivingBeing person) {
		return nearestFarm.getMapData(VConstants.gate).getListCreate(VConstants.hatred).contains(getHome(person).getPosition());
	}
			...
			if(VConstants.getRandom().nextDouble() < conflict){
				HashMapData hmd=fullMapData.getNearby(person, new GetForNearby<HashMapData>(fullMapData) {
					public HashMapData get(HashMapData hashmapdata) {
						if(hashmapdata.getLivingBeing() != null&&PeopleRule.isHuman(hashmapdata.getLivingBeing())&&!hashmapdata.getLivingBeing().equals(person)){
							return hashmapdata;									
						}
						return null;
					}
	...
	public boolean run(PBase p, HashMapData hmd, FullMapData fmd) {
		...
		//if that radius is a good spot (random chance) then have them move back
		//and start secondary quest of getting wives and starting a town
		//also do a random chance of killing some of them off 		
		//if random says no the living either come back in disgrace or become hermits
			...
			public Returnable execute(FullMapData fullMapData, LivingBeing person) {
				...
				HashMapData hmd=fullMapData.getNearby(person, new GetForNearby<HashMapData>(fullMapData) {
					...
					public HashMapData get(HashMapData hashmapdata) {
						...
						if(!hashmapdata.isBlock()&&VConstants.getRandom().nextDouble() < .05){//
							return hashmapdata;
						}
						return null;
					}
				...
				if(hmd != null){
					OobList oobList = new OobList(new Move(hmd, "raid",moveCombat),new Wait("happy", 1));
					...
					if(VConstants.getRandom().nextDouble() < .99){
						...
						if(fullMapData.getNearKeyValue(VConstants.gate, SConstants.farm, hmd, 3) == null){
	...
	protected static void setHome(LivingBeing person) {
		person.getPopulation().put(VConstants.home, getNearestVillage(person).getPosition());
	}
	...
	public static HashMapData getNearestVillage(LivingBeing person) {
		return person.getParent().getParent().getNearKeyValue(VConstants.gate, SConstants.farm, person, 3);
	}
}
